...
class Worker {
  private:
    ...
    // Name of the library that implements the components executed by this
    // worker
    std::string componentLibName;

    // List of components (Ids) that were completed the execution within this
    // Worker. The Resource Manager computing threads will insert items to this
    // list using the call back function that is executed when all tasks
    // associated to a given component have been successfully executed. The main
    // Worker controlling threads will keep checking whether elements were added
    // to this list, and notify the Manger when so.
    ...
    // Receive information about this execution, load library, and prepare to
    // start computation
    void configureExecutionEnvironment();

    // Receive data describing a pipeline component instantiation that should be
    // executed
    PipelineComponentBase *receiveComponentInfoFromManager();

    void receiveCacheInfoFromManager();

    // Send message to Manager notifying it about the end of components, if
    // there are any in the list of computedComponents
    ...
    // This class is responsible for deleting a given component
    // PipelineComponentBase from the map of active ones
  ...
  public:
    ...
    // Retrieve the name of the library that implements the components used by
    // this worker
    ...
    // Add a given component id to the output list of already computed component
    // instances
    ...
    // Increments a counter responsible to hold the number of component
    // instances active with the current Worker
    ...
    void printHello() {
        std::cout << "Hey! componentLibName: " << this->getComponentLibName()
                  << std::endl;
    };
    void setCommWorld(MPI_Comm commWorld) { this->comm_world = commWorld; }
};
