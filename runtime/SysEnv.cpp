...
SysEnv::SysEnv() { this->manager = NULL; }
...
// initialize MPI
MPI_Comm init_mpi(int argc, char **argv, int &size, int &rank,
                  std::string &hostname) {
    int initialized;
...
Manager *SysEnv::getManager() const { return manager; }

void SysEnv::setManager(Manager *manager) { this->manager = manager; }
            ...
            switch (argv[i][1]) {
                ...
                case 's':
                    ...
                    if (!policyArgumentValue.compare("priority")) {
                        ...
                        std::cout << "Unknow policy: " << policyArgumentValue
                                  << ". Using FCFS." << std::endl;
                    }
                    break;
                case 'r':
...
int SysEnv::startupSystem(int argc, char **argv,
                          std::string componentsLibName) {
    // set up mpi
    ...
    std::cout << "Using: cpus=" << this->cpus << " gpus=" << this->gpus
              << " policy=" << this->policy
              << " dataLocalityAware?=" << dataLocalityAware << std::endl;
    if (size == 1) {
        printf(
            "ERROR: this must run with 2 or more MPI processes. 1 for the "
            "Manager and 1(+) for Worker(s)\n");
        MPI_Finalize();
        exit(1);
        return -4;
    }
    ...
    if (rank == manager_rank) {
        ...
        this->setManager(new Manager(comm_world, manager_rank, worker_size,
                                     componentDataAwareSchedule,
                                     managerQueueType, cpus, gpus));

        // Check whether all Worker have successfully initialized their
        // execution
        ...
        // Create one worker object for each Worker process. The Worker objects
        // follows the singleton pattern and, as such, only the existing
        // instance is retrieved through the getInstance class
        Worker *localWorker = Worker::getInstance(
            manager_rank, rank, windowSize, cpus, gpus, withHybridWorkarround,
            policy, dataLocalityAware, prefetching,
            cacheOnRead);  // new Worker(comm_world, manager_rank, rank,
                           // windowSize, cpus, gpus, policy, dataLocalityAware,
                           // prefetching );
...
int SysEnv::startupExecution() {
    ...
    std::cout << "MANAGER " << this->getManager()->getManagerRank()
              << ": FINISHED in " << t2 - t1 << " us" << std::endl;
    return 1;
}

int SysEnv::finalizeSystem() { return this->getManager()->finalizeExecution(); }
