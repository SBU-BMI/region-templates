...
#endif
	// space to pack data size, data, and component id.
	int componentSize = this->getCompInst()->getResultDataSize() * sizeof(char) + 2 * sizeof(int);
	char *compDataToManager = (char*)malloc(componentSize*sizeof(char));

	// copy id, result data size, and result data
	((int*)compDataToManager)[0] = this->getCompInst()->getId();
	((int*)compDataToManager)[1] = this->getCompInst()->getResultDataSize();
...
//		std::cout << std::endl<< "This is a region template component base" << std::endl << std::endl;
		...
		curWorker->addComputedComponent(compDataToManager, size_bytes, data);
#else
