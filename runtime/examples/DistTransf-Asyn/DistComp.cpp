...
//	(*maskImage).copyTo((*maskCopy));
	...
	for(int tileY=0; tileY < nTilesY; tileY++){
		for(int tileX=0; tileX < nTilesX; tileX++){
			...
			Mat roiNeighbor((*neighborImage), Rect(tileX*tileSize, tileY*tileSize , tileSize, tileSize));

			TaskArgumentMat *maskArg = new TaskArgumentMat();;
			maskArg->cpu_mat = roiMask;
			TaskArgumentMat *neighborArg = new TaskArgumentMat();;
			neighborArg->cpu_mat = roiNeighbor;
			neighborArg->setType(ExecEngineConstants::OUTPUT);

			TaskTileProp *taskTileProp = new TaskTileProp(neighborImage, maskImage, tileX, tileY, tileSize, mask.cols);
			taskTileProp->addArgument(maskArg);
			taskTileProp->addArgument(neighborArg);
