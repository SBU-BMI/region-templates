...
RegionTemplate::~RegionTemplate() {
    ...
    // cout << "====== DELETING RT " << id << ":" << name << "\n";
    for (auto it : this->templateRegions) {
...
const BoundingBox &RegionTemplate::getBb() const { return bb; }

void RegionTemplate::setBb(const BoundingBox &bb) { this->bb = bb; }

std::string RegionTemplate::getName() const { return name; }

void RegionTemplate::setName(std::string name) { this->name = name; }

unsigned int RegionTemplate::getVersion() const { return version; }

int RegionTemplate::getTimestamp() const { return timestamp; }

bool RegionTemplate::insertDataRegion(DataRegion *dataRegion) {
    bool retValue = true;
#ifdef DEBUG
    std::cout << "DR: " << dataRegion->getName()
              << " time: " << dataRegion->getTimestamp()
              << " version: " << dataRegion->getVersion() << std::endl;
#endif
    // it starts w/ false when component is created, and is changed to true when
    // user inserts a data region.
        ...
        if (it != templateRegions.end()) {
            ...
            for (; listIt != (*it).second.end(); listIt++) {
                if (dataRegion->getTimestamp() == (*listIt)->getTimestamp() &&
                    ...
                    std::cout << "Warning: DataRegion with same version and "
                                 "timestamp is stored. It will be deleted"
                              << std::endl;
                    delete (*listIt);
            ...
            // I expect this data region to be accessed soon after inserted
            // here. Putting it into the head of the list
            (*it).second.push_front(dataRegion);
        } else {
            std::list<DataRegion *> dataRegionList;
            dataRegionList.push_back(dataRegion);
            templateRegions.insert(
                std::pair<std::string, std::list<DataRegion *>>(
                    dataRegion->getName(), dataRegionList));
        }

    } else {
        // If the name(ID) of the region is empty there is nothing we can do.
        std::cout << "Warning: tried to insert a data region with empty name "
                     "in a region template. It was not inserted!"
                  << std::endl;
        retValue = false;
    }
    return retValue;
...
DataRegion *RegionTemplate::getDataRegion(std::string drName, std::string drId,
                                          int timestamp, int version) {
    // Did not find the region requested by default
    DataRegion *retValue = NULL;

    // friendly iterator to walk through the list of regions with same name
    // (equals to parameter 'name')
    std::map<std::string, std::list<DataRegion *>>::iterator it;
    ...
    if (it != templateRegions.end()) {
        // try to find a region with same version and timestamp. This should be
        // a happy matching
        std::list<DataRegion *>::iterator listIt = (*it).second.begin();

        // check if a region with same version and timestamp exists
        for (; listIt != (*it).second.end(); listIt++) {
            //			std::cout << " Number of data regions: "<<
            //(*it).second.size() << std::endl;
            // if found the specific region. If drId is empty, any id is
            // accepted
            if ((drId.compare((*listIt)->getId()) == 0 || drId.empty()) &&
                ...
                retValue = (*listIt);
                //				std::cout << "found: "<<
                // retValue->getName() << " "<< retValue->getId() <<" time: "<<
                // retValue->getTimestamp()<< " version: "<<
                // retValue->getVersion()<< std::endl;
                // if required data region has not been loaded
                if (retValue->empty() && (this->getLocation() ==
                                          PipelineComponentBase::WORKER_SIDE)) {
...
#ifdef DEBUG
                        std::cout
                            << "auxCache->getDR: " << this->getName() << ":"
                            << this->getId() << ":" << retValue->getName()
                            << ":" << retValue->getId() << ":"
                            << retValue->getTimestamp() << ":"
                            << retValue->getVersion() << ":" << true << ":"
                            << auxCache->getWorkerId() << ":"
                            << retValue->getWorkerId() << std::endl;
#endif

                        retValue = auxCache->getDR(this->getName(),
                                                   this->getId(), retValue);
                        // , retValue->getName(),
                        //   retValue->getId(), retValue->getInputFileName(),
                        //   retValue->getTimestamp(), retValue->getVersion(),
                        //   retValue->getType(), true,
                        //   retValue->getIsAppInput(),
                        //   retValue->getInputFileName(), retValue->isSvs(),
                        //   retValue->getRoi());

                    } else {
                        std::cout
                            << "RT::getDataRegion: CACHE is NULL. rtName: "
                    ...
                    // std::cout << "Read Time: "<< retValue->getName() << "
                    // "<<endRead-initRead<< std::endl;
                }
                break;
            }
        }
...
// This function is partially implemented. It is not serializing all the data
// fields
int RegionTemplate::serialize(char *buff) {
    ...
    memcpy(buff + serialized_bytes, this->name.c_str(),
    ...
    // pack the id itself
    memcpy(buff + serialized_bytes, this->getId().c_str(),
           sizeof(char) * nameSize);
    serialized_bytes += nameSize * sizeof(char);
    ...
    for (; it != templateRegions.end(); it++) {
        // Here we have each entry of the map with the same name. However, there
        // may be several regions w/ the same name that differ in timestamp and
        // version
        std::list<DataRegion *>::iterator listIt = (*it).second.begin();

        // Walk through all the data regions w/ the same name and get their
        // sizes.
        for (; listIt != (*it).second.end(); listIt++) {
...
int RegionTemplate::deserialize(char *buff) {
    ...
    for (int i = 0; i < regionSize; i++) {
        ...
        //		std::cout << "\t\t deserialized: before: " <<
        // deserialized_bytes << std::endl;
        deserialized_bytes += ddr2d->deserialize(buff + deserialized_bytes);
        //		std::cout << "\t\t deserialized: after: " <<
        // deserialized_bytes << std::endl;
...
std::string RegionTemplate::getId() const { return id; }

void RegionTemplate::setId(std::string id) { this->id = id; }
...
int RegionTemplate::getNumDataRegions() {
    ...
    // walk through data regions w/ same name and get size of each of those
    // lists
...
DataRegion *RegionTemplate::getDataRegion(int index) {
    ...
    if (index >= 0 && index < this->getNumDataRegions()) {
        ...
        for (; it != this->templateRegions.end(); it++) {
            //			std::cout << "index: " << index << " baseIndex:
            //"<< baseIndex << " (*it).second.size: "<< (*it).second.size() <<
            // std::endl;
    ...
    for (; it != this->templateRegions.end(); it++) {
        ...
        for (; listIt != (*it).second.end(); listIt++) {
            ...
            /*		std::cout << "\t\t name: "<< dr->getName() << std::endl;
                            ...
                            std::cout << "\t\t version: "<< dr->getVersion() <<
               std::endl; std::cout << "\t\t timestamp: "<< dr->getTimestamp()
               << std::endl; std::cout << "\t\t inType: "<< dr->getInputType()
               << std::endl;
                            std::cout << "\t\t outType: "<< dr->getOutputType()
               << std::endl;*/
        }
    }
...
// Get number of regions with a given name which differ in temporal component
// and version
...
bool RegionTemplate::isLazyRead() const { return lazyRead; }

int RegionTemplate::getLocation() const { return location; }

void RegionTemplate::setLocation(int location) { this->location = location; }

void RegionTemplate::setLazyRead(bool lazyRead) { this->lazyRead = lazyRead; }

void RegionTemplate::setCache(Cache *cache) { this->cachePtr = cache; }

Cache *RegionTemplate::getCache(void) { return this->cachePtr; }
            ...
            if ((drId.compare((*listIt)->getId()) == 0 || drId.empty()) &&
                ...
                if (retValue->empty() && (this->getLocation() ==
                    ...
                    if (auxCache != NULL) {
#ifdef DEBUG
                        std::cout
                            << "auxCache->deleteDr: " << this->getName() << ":"
                            << this->getId() << ":" << retValue->getName()
                            << ":" << retValue->getId() << ":"
                            << retValue->getTimestamp() << ":"
                            << retValue->getVersion() << ":" << true << ":"
                        ...
                        auxCache->deleteDR(
                            this->getName(), this->getId(), retValue->getName(),
                            retValue->getId(), retValue->getTimestamp(),
                            retValue->getVersion());
