...
PrePartThresBGCostFunction::PrePartThresBGCostFunction(int bgThr, int dilate,
    ...
    auto costFunc     = new ThresholdBGCostFunction(bgThr, dilate, erode);
    this->costArray =
...
double PrePartThresBGCostFunction::cost(cv::Mat img) const {
...
cv::Mat PrePartThresBGCostFunction::costImg(cv::Mat img) const {
...
double PrePartThresBGCostFunction::cost(cv::Mat            _img,
                                        cv::Rect_<int64_t> tile) const {
    return this->cost(_img, tile.y, tile.y + tile.height, tile.x,
                      tile.x + tile.width);
}

double PrePartThresBGCostFunction::cost(cv::Mat _img, int64_t yi, int64_t yo,
    ...
    // std::cout << xo << "\n";
    cv::Mat subMat =
        this->costArray(cv::Range(yiTile, yoTile), cv::Range(xiTile, xoTile));
    // std::cout << "============2===========\n";
