    ...
    for (std::string img : this->initialPaths) {
        ...
        switch (this->tilingAlg) {
            case FIXED_GRID_TILING: {
                ...
                //     << std::endl;
                fixedGrid(this->nGpuTiles, gw, gh, gmw, gmh, curTiles);
                ...
                //     << std::endl;
                fixedGrid(this->nCpuTiles, cw, ch, cmw, cmh, curTiles);
            ...
            case LIST_ALG_EXPECT: {
                ...
                for (std::list<rect_t>::iterator r=tiles.begin(); 
                        r!=tiles.end(); r++) {
                    curTiles.push_back(cv::Rect_<int64_t>(
