...
void intv(std::vector<int>& fs, std::vector<int>& out, int prev, int s,
          int ii) {
    int f = 1;
    if (s > 0) {
...
int fixedGrid(int64_t nTiles, int64_t w, int64_t h, int64_t mw, int64_t mh,
    ...
    for (int ti = 0; ti < yTiles; ti++) {
        for (int tj = 0; tj < xTiles; tj++) {
            ...
            cv::Rect_<int64_t> roi(tjTmp, tiTmp, tjjTmp, tiiTmp);
            rois.push_back(roi);

#ifdef DEBUG
            std::cout << "creating regular roi " << roi.x << "+" << roi.width
