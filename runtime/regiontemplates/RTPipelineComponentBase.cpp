...
RTPipelineComponentBase::~RTPipelineComponentBase() {
    // Delete associated region templates: this is wrong. Other components also
    // access the same region templates
    if (this->getLocation() == PipelineComponentBase::WORKER_SIDE) {
#ifdef DEBUG
        std::cout << "~RTPipelineComponentBase workerside: nRegionTemplates: "
                  << this->regionTemplates.size() << std::endl;
#endif
        std::map<std::string, RegionTemplate *>::iterator rtIt =
            this->regionTemplates.begin();
...
int RTPipelineComponentBase::serialize(char *buff) {
    ...
    std::set<std::pair<std::string, std::string>>::iterator irIt =
        input_data_regions.begin();
    ...
    for (int i = 0; i < num_input_regions; i++, irIt++) {
        ...
        // pack string
        memcpy(buff + serialized_bytes, rtName.c_str(),
               sizeof(char) * rtNameSize);
        ...
        // pack string
        memcpy(buff + serialized_bytes, drName.c_str(),
               sizeof(char) * drNameSize);
    ...
    std::set<std::pair<std::string, std::string>>::iterator orIt =
        output_data_regions.begin();
    ...
    //	std::cout << "Serialize numRTInsntaces:" << nRegionTemplateInsts <<
    // std::endl;
...
int RTPipelineComponentBase::deserialize(char *buff) {
    //	std::cout << "\t THIS IS RT_PIPELINE_COMPONENT:deserialize" <<
    // std::endl;
    ...
    for (int i = 0; i < num_input_regions; i++) {
        ...
        this->addInputOutputDataRegion(rtName, drName,
    ...
    deserialized_bytes += sizeof(int);
    //	std::cout << "Deserialize numRTInsntaces:" << nRegionTemplateInsts <<
    // std::endl;
    ...
    for (int i = 0; i < nRegionTemplateInsts; i++) {
        ...
        //		std::cout << "Deserialize: add RT: "<<rt->getName() <<
        // std::endl;
    }
    ...
    for (int i = 0; i < num_rts_names; i++) {
        ...
        memcpy(rt_name, buff + deserialized_bytes,
               sizeof(char) * rts_name_size);
    ...
    //	std::cout << "\t THIS IS END RT_PIPELINE_COMPONENT:deserialize" <<
    // std::endl;
...
RegionTemplate *
RTPipelineComponentBase::getRegionTemplateInstance(std::string dataRegionName) {
    ...
    // std::cout << "finding " << dataRegionName << " = " << retRegionTemplate
    // << std::endl;
    return retRegionTemplate;
}

int RTPipelineComponentBase::addRegionTemplateInstance(RegionTemplate *rt,
                                                       std::string     name) {
    // set cache Ptr Value
    if (rt != NULL) {
...
#ifdef DEBUG
    std::cout << "instantiateRegionTemplates..Number of input data regions"
              << this->input_data_regions.size() << std::endl;
    std::cout << "instantiateRegionTemplates..Number of region templates: "
              << this->regionTemplates.size() << std::endl;
#endif
    //	DataRegionFactory drf;
    std::set<std::pair<std::string, std::string>>::iterator inputDataRegionsIt =
        this->input_data_regions.begin();

    long long initStage = Util::ClockGetTime();

    // walk through input data regions/region templates and instantiate them
    for (; inputDataRegionsIt != this->input_data_regions.end();
         inputDataRegionsIt++) {
        ...
        if (rt != NULL) {
            ...
            if (drIt != rt->templateRegions.end()) {
                // Iterate over all the data regions, regardless of timestamp
                // and version
                std::list<DataRegion *>::iterator listIt =
                    (*drIt).second.begin();
                for (; listIt != (*drIt).second.end(); listIt++) {
                    DataRegion *dr = (*listIt);
#ifdef DEBUG
                    std::cout << "Instantiate data region: " << rtName << ":"
                              << drName << " timestamp: " << dr->getTimestamp()
                              << std::endl;
#endif
                    ...
                    if (auxCache != NULL) {
                        rt->getCache()->insertDR(rt->getName(), rt->getId(), dr,
                                                 true);
#ifdef DEBUG
                        std::cout << "Stage data region into cache: " << rtName
                                  << ":" << drName
                                  << " timestamp: " << dr->getTimestamp()
                                  << " version:" << dr->getVersion()
                                  << " id: " << dr->getId() << std::endl;
#endif
                    }
                    //					}
                }
            ...
            std::cout << "Warning: could not find input regionTemplate "
                      << rtName << std::endl;
        }
    }
...
RegionTemplate *RTPipelineComponentBase::getRegionTemplateInstance(int index) {
    ...
    if (index >= 0 && index < this->getNumRegionTemplates()) {
        std::map<std::string, RegionTemplate *>::iterator itRT =
            this->regionTemplates.begin();
...
void RTPipelineComponentBase::updateRegionTemplateInfo(RegionTemplate *rt) {
    ...
    if (curRt != NULL) {
        ...
        for (int i = 0; i < rt->getNumDataRegions(); i++) {
            ...
            DataRegion *drCur =
                curRt->getDataRegion(dr->getName(), dr->getId(),
                                     dr->getTimestamp(), dr->getVersion());
            if (drCur == NULL) {
                // create data region;
            ...
            // if (dr->isSvs())
            drCur->setRoi(dr->getRoi());

            if (dr->aborted())
                drCur->abort();
#ifdef DEBUG
            std::cout << "CompName: " << this->getComponentName()
                      << " Updating region template: " << rt->getName()
                      << " data region: " << dr->getName()
                      << " dr->id: " << drCur->getId()
                      << " dr->timestamp: " << dr->getTimestamp()
                      << " dr->version: " << dr->getVersion()
                      << " componentId: " << this->getId() << std::endl;
#endif
        }
    } else {
        std::cout << "Warning: failed to update region template: "
                  << rt->getName() << std::endl;
    }
#ifdef DEBUG
...
void RTPipelineComponentBase::addInputOutputDataRegion(
    ...
    switch (type) {
        case RTPipelineComponentBase::INPUT:
            this->input_data_regions.insert(
                std::make_pair<std::string, std::string>(
                    (std::string)regionTemplateName,
                    (std::string)dataRegionName));
            break;
        case RTPipelineComponentBase::OUTPUT:

            this->output_data_regions.insert(
                std::make_pair<std::string, std::string>(
                    (std::string)regionTemplateName,
                    (std::string)dataRegionName));
            break;
        ...
        case RTPipelineComponentBase::INPUT_OUTPUT:
            ...
            break;
        default:
            std::cout << "Error: unknown data region relation type (input, "
                         "ouput,etc): "
                      << type << std::endl;
            break;
    }
...
int RTPipelineComponentBase::stageRegionTemplates() {
#ifdef DEBUG
    std::cout << "Number of output data regions"
              << this->output_data_regions.size() << std::endl;
#endif
    long long initStage = Util::ClockGetTime();

    // Check if the region templates need to be staged. if any data region is
    // null, it is not staged.
    std::set<std::pair<std::string, std::string>>::iterator
    ...
    for (; outputDataRegionsIt != this->output_data_regions.end();
         outputDataRegionsIt++) {
        ...
        if (rt != NULL) {
            // friendly iterator to walk through the list of regions with same
            // name (equals to parameter 'drName')
            std::map<std::string, std::list<DataRegion *>>::iterator it =
                rt->templateRegions.find(drName);
            ;

            // if we have found regions with that name
            if (it != rt->templateRegions.end()) {
#ifdef DEBUG
                std::cout << "CallBack: #drs " << rt->templateRegions.size()
                          << std::endl;
#endif
                ...
                std::cout << "Warning: could not find output "
                             "regionTemplate:dataRegion "
                          << rtName << ":" << drName << std::endl;
            }
        } else {
            std::cout << "Warning: could not find output regionTemplate "
                      << rtName << std::endl;
        }
    }
    long long endStage = Util::ClockGetTime();
...
int RTPipelineComponentBase::createIOTask() {
    // this was used to perform IO in background, but it makes IO operations
    // less dynamic. Since it need to read all data regions with a given name.
    // There was no way to detetermine specific version/timestamp
...
void RTPipelineComponentBase::setCache(Cache *cache) {
    ...
    for (int i = 0; i < this->getNumRegionTemplates(); i++) {
        ...
        // std::cout << "setCache: "<< aux->getName()<< " isCacheNull?: "<<
        // (this->getCache()==NULL)<<std::endl;
