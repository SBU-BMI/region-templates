...
PARAMOPTS="-i 2 -j 2 -p st_intersects -a $mask -b $referenceMask"
...
${hadoopgisBuildPath}bin/compute_partition_size 4000 1.0 ${total_size} ${object_count} > ${mask}partition_size.cfg
...
PARAMOPTS5="-o 3 "${PARAMOPTS}" -f intersect -c ${mask}denormpartition.idx"
#1:1,2:1,area1,area2
#echo ${PARAMOPTS5}
