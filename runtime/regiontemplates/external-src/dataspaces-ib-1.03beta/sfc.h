...
inline static bitmask_t getIntBits(unsigned nDims, unsigned nBytes, char const *c, unsigned y)
{
...
#include <string.h>
inline static void propogateIntBits(unsigned d, unsigned nBytes, char *c, unsigned y, int fold)
{
...
inline static bitmask_t getIEEESignBits(unsigned nDims, double const *c)
{
	unsigned d;
	ieee754_double x;
	bitmask_t bits = 0;
...
inline static bitmask_t getIEEEBits(unsigned nDims, unsigned ignoreMe,	/* ignored */
				    char const *cP, unsigned y)
...
inline static void propogateIEEEBits(unsigned d, unsigned nBytes, char *cP, unsigned y, int fold)
{
...
inline static unsigned getIEEEexptMax(unsigned nDims, double const *c)
{
	unsigned max = 0;
	unsigned d;
	for(d = 0; d < nDims; ++d) {
...
inline static void getIEEEinitValues(double const *c1, unsigned y, unsigned nDims, unsigned *rotation, bitmask_t * bits, bitmask_t * index)
{
...
inline static int hilbert_cmp_work(unsigned nDims, unsigned nBytes, unsigned nBits, unsigned max, unsigned y, char const *c1, char const *c2, unsigned rotation, bitmask_t bits, bitmask_t index, BitReader getBits)
{
	bitmask_t const one = 1;
...
inline int hilbert_cmp(unsigned nDims, unsigned nBytes, unsigned nBits, void const *c1, void const *c2)
{
	bitmask_t const one = 1;
	bitmask_t bits = one << (nDims - 1);
	return hilbert_cmp_work(nDims, nBytes, nBits, 0, nBits, (char const *) c1, (char const *) c2, 0, bits, bits, getIntBits);
}

inline int hilbert_ieee_cmp(unsigned nDims, double const *c1, double const *c2)
{
	...
	return hilbert_cmp_work(nDims, 8, 64, max, max + 53, (char const *) c1, (char const *) c2, rotation, bits, index, getIEEEBits);
}
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
inline static unsigned hilbert_box_vtx_work(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, unsigned max, unsigned y, char *c1, char *c2, unsigned rotation, bitmask_t bits, bitmask_t index, BitReader getBits)
{
	bitmask_t const one = 1;
...
inline unsigned hilbert_box_vtx(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, void *c1, void *c2)
{
	bitmask_t const one = 1;
	bitmask_t bits = one << (nDims - 1);
...
inline unsigned hilbert_ieee_box_vtx(unsigned nDims, int findMin, double *c1, double *c2)
{
	unsigned rotation, max;
	bitmask_t bits, index;
	if(getIEEESignBits(nDims, c1) != getIEEESignBits(nDims, c2))
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
inline unsigned hilbert_box_pt_work(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, unsigned max, unsigned y, char *c1, char *c2, unsigned rotation, bitmask_t bits, bitmask_t index, BitReader getBits, BitWriter propogateBits)
{
	bitmask_t const one = 1;
...
inline unsigned hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, void *c1, void *c2)
{
	bitmask_t const one = 1;
	bitmask_t bits = one << (nDims - 1);
	return hilbert_box_pt_work(nDims, nBytes, nBits, findMin, 0, nBits, (char *) c1, (char *) c2, 0, bits, bits, getIntBits, propogateIntBits);
}

inline unsigned hilbert_ieee_box_pt(unsigned nDims, int findMin, double *c1, double *c2)
{
	unsigned rotation, max;
	bitmask_t bits, index;
	...
	if(c1Signs != c2Signs) {
		...
		index = 1;
		hilbert_box_pt_work(nDims, 8, 64, findMin, IEEErepBits - 1, IEEErepBits, (char *) c1, (char *) c2, rotation, bits, index, getIEEEBits, propogateIEEEBits);
	}
	...
	return hilbert_box_pt_work(nDims, 8, 64, findMin, max, max + 53, (char *) c1, (char *) c2, rotation, bits, index, getIEEEBits, propogateIEEEBits);
}
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
inline int hilbert_nextinbox(unsigned nDims, unsigned nBytes, unsigned nBits, int findPrev, void *c1V, void *c2V, void const *ptV)
{
	bitmask_t const one = 1;
	unsigned y = nBits;
	bitmask_t const ndOnes = ones(bitmask_t, nDims);
...
inline void hilbert_incr(unsigned nDims, unsigned nBits, bitmask_t coord[])
{
	bitmask_t const one = 1;
	bitmask_t const ndOnes = ones(bitmask_t, nDims);
	bitmask_t const nd1Ones = ndOnes >> 1;
