...
DataRegion::~DataRegion() {
    // std::cout << "+++++++++++ DELETING DR: " << this->getName() << "\n";
    // std::cout << boost::stacktrace::stacktrace() << std::endl;
}

int DataRegion::getType() const { return regionType; }

const BoundingBox &DataRegion::getBb() const { return bb; }

void DataRegion::setBb(const BoundingBox &bb) { this->bb = bb; }

void DataRegion::setName(std::string name) { this->name = name; }

int DataRegion::getResolution() const { return resolution; }

void DataRegion::setResolution(int resolution) {
    this->resolution = resolution;
}

int DataRegion::getTimestamp() const { return timestamp; }

void DataRegion::setTimestamp(int timestamp) { this->timestamp = timestamp; }

int DataRegion::getVersion() const { return version; }

std::string DataRegion::getName() const { return name; }

void DataRegion::setVersion(int version) { this->version = version; }

void DataRegion::setType(int type) { this->regionType = type; }

void DataRegion::setSvs() { this->svs = true; }

bool DataRegion::isSvs() { return this->svs; }

bool DataRegion::write() { return false; }

bool DataRegion::instantiateRegion() { return false; }
...
int DataRegion::serialize(char *buff) {
    ...
    memcpy(buff + serialized_bytes, this->getName().c_str(),
    ...
    memcpy(buff + serialized_bytes, this->getInputFileName().c_str(),
           sizeof(char) * inputFileNameSize);
    ...
    std::vector<std::pair<BoundingBox, std::string>>::iterator bb2IdIt =
        bb2Id.begin();
    while (bb2IdIt != bb2Id.end()) {
        ...
        memcpy(buff + serialized_bytes, (*bb2IdIt).second.c_str(),
               (*bb2IdIt).second.size() * sizeof(char));
...
void DataRegion::setRoi(cv::Rect_<int64_t> roi) { this->roi = roi; }

cv::Rect_<int64_t> DataRegion::getRoi() { return this->roi; }
...
int DataRegion::getInputType() const { return inputType; }

void DataRegion::setInputType(int inputType) { this->inputType = inputType; }

int DataRegion::getOutputType() const { return outputType; }

std::string DataRegion::getId() const { return id; }

void DataRegion::setId(std::string id) { this->id = id; }

bool DataRegion::empty() { return false; }
...
const BoundingBox &DataRegion::getROI() const { return ROI; }

void DataRegion::setROI(const BoundingBox &roi) { ROI = roi; }
...
std::pair<BoundingBox, std::string> DataRegion::getBB2IdElement(int index) {
    if (index < 0 || index > this->bb2Id.size()) {
        std::cout << "Trying to access chunked data out of bounds. Index: "
                  << index << ", but number of BBs: " << bb2Id.size();
        BoundingBox emptyBB;
        std::string emptyStr;
...
int DataRegion::getBB2IdSize() { return this->bb2Id.size(); }

int DataRegion::getOutputExtension() const { return outputExtension; }

void DataRegion::setOutputExtension(int outputExtension) {
    this->outputExtension = outputExtension;
}

DataRegion *DataRegion::clone(bool copyData) { return NULL; }

long DataRegion::getDataSize() { return -1; }

std::string DataRegion::getInputFileName() const { return inputFileName; }

void DataRegion::setInputFileName(std::string inputFileName) {
    this->inputFileName = inputFileName;
}

bool DataRegion::getIsAppInput() const { return isAppInput; }

int DataRegion::getCacheLevel() const { return cacheLevel; }

void DataRegion::setCacheLevel(int cacheLevel) {
    this->cacheLevel = cacheLevel;
}

int DataRegion::getCacheType() const { return cacheType; }

void DataRegion::setCacheType(int cacheType) { this->cacheType = cacheType; }

int DataRegion::getWorkerId() const { return workerId; }

void DataRegion::setWorkerId(int workerId) { this->workerId = workerId; }

long DataRegion::getCachedDataSize() const { return cachedDataSize; }

bool DataRegion::isModified() const { return modified; }

void DataRegion::setModified(bool modified) { this->modified = modified; }
...
int DataRegion::getStorageLevel() const { return storageLevel; }

void DataRegion::setStorageLevel(int storageLevel) {
    this->storageLevel = storageLevel;
}

bool DataRegion::aborted() { return this->wasAborted; }

void DataRegion::abort() { this->wasAborted = true; }
