...
DenseDataRegion2D::~DenseDataRegion2D() {
	...
	for(it = this->chunkedDataCaching.begin(); it != this->chunkedDataCaching.end(); it++){
		it->second.release();
	}
#ifdef DEBUG
...
void DenseDataRegion2D::allocData(int x, int y, int type, int device) {
	...
	this->cachedDataSize = this->dataCPU.rows * this->dataCPU.cols * this->dataCPU.elemSize();
}
...
cv::Mat DenseDataRegion2D::getData() {
	return this->dataCPU;
}

cv::Mat* DenseDataRegion2D::getData(int x, int y, int width, int height, bool cacheData) {
	cv::Mat* ret = NULL;

	// create a bounding box w/ the area requested by the user
	BoundingBox userROI(Point(x, y, 0), Point(x+width-1, y+height-1, 0));

	// if requested area is the default (x=-1 or y=-1), change it to be the data region ROI
	if(x < 0 || y < 0){
		userROI = this->getROI();
		std::cout << "ROI:";
		this->getROI().print();
	...
	for(int i = 0; i < this->getBB2IdSize(); i++){
		std::pair<BoundingBox, std::string> chunkInfo = this->getBB2IdElement(i);

		// check if BB intersects ROI
		if(userROI.doesIntersect(chunkInfo.first)){
			// check if BB is already cached.
			std::map<BoundingBox, cv::Mat>::iterator it = this->chunkedDataCaching.find(chunkInfo.first);
			...
			if(it != this->chunkedDataCaching.end()){
				...
				if(ret == NULL){
					ret = new cv::Mat(cv::Size(userROI.sizeCoordX(), userROI.sizeCoordY()), currentChunk.type());
				}

				// Copy intersecting region to user data
				BoundingBox intersection = userROI.intersection(chunkInfo.first);


				// set ROI in the current data chunk:
				int intersectionWidth = intersection.getUb().getX() - intersection.getLb().getX() +1;
				int intersectionHeight = intersection.getUb().getY() - intersection.getLb().getY() +1;

				// the cv::Mat start at (0,0) but the intersection is global, so we have to shift the intersection by the ROI size
				cv::Rect curROI(intersection.getLb().getX()-chunkInfo.first.getLb().getX(), intersection.getLb().getY()-chunkInfo.first.getLb().getY(), intersectionWidth, intersectionHeight);
				// create a header w/ the ROI
				cv::Mat currentChunkROI = currentChunk(curROI);

				// create a ROI for the user-requested region, using the same procedure
				cv::Rect retROI(intersection.getLb().getX()-userROI.getLb().getX(), intersection.getLb().getY()-userROI.getLb().getY(), intersectionWidth, intersectionHeight);
				// create a header for the user data mat
				cv::Mat retDataROI = (*ret)(retROI);
				currentChunkROI.copyTo(retDataROI);

				// we're not caching data this time, delete loaded chunk from cache
				if(cacheData == false){
...
int DenseDataRegion2D::getXDimensionSize() {
	return this->dataCPU.cols;
}
...
void DenseDataRegion2D::deleteChunkFromCache(BoundingBox boundingBox) {
	std::map<BoundingBox, cv::Mat >::iterator it = this->chunkedDataCaching.find(boundingBox);
	if(it != this->chunkedDataCaching.end()){
		it->second.release();
...
int DenseDataRegion2D::getYDimensionSize() {
	return this->dataCPU.rows;
}
