...
map<int, ArgumentBase*> get_workflow_inputs(FILE* workflow);
...
int main() {
	...
	map<int, ArgumentBase*> workflow_inputs = get_workflow_inputs(workflow);
	cout << "inputs:" << endl;
	for(map<int, ArgumentBase*>::iterator i=workflow_inputs.begin(); i!=workflow_inputs.end(); i++)
		cout << i->first << ":" << (i->second)->getName() << ": " << (i->second)->toString() << endl;
	cout << endl;
	...
	// int workflow_id = 0;
	// generate_workflow_stages(stages, workflow_inputs, all_tasks);
...
void generate_workflow_stages(map<string, RTPipelineComponentBase*> &stages, map<string, 
	ArgumentBase*> &arguments, list<task_t> tasks) {

	for (list<task_t>::iterator i=tasks.begin(); i!=tasks.end(); i++) {
		// check if the stage wasn't already created
		if (stages.find(i->name) != stages.end()) {
			// create stage
			PipelineComponentBase* executable = PipelineComponentBase::ComponentFactory::getComponentFactory(i->name)();

			// create output arguments
			for (list<port_t>::iterator j=i->outputs.begin(); j!=i->outputs.end(); j++) {
				// arguments[j->] = generate_argument()
			}
