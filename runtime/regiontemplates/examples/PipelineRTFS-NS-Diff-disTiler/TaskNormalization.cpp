...
bool TaskNormalization::run(int procType, int tid) {
    ...
    // cv::Mat inputImage = ((DenseSvsDataRegion2D*)this->raw)->getData();
	...
	if(inputImage.rows > 0)
		NormalizedImg = norm(inputImage, targetMean, targetStd);
	else
...
cv::Mat segFG(cv::Mat I, cv::Mat M)
{
...
void PixelClass(cv::Mat I, cv::Mat o_fg, cv::Mat o_bg, cv::Mat& o_fg_lab, cv::Mat& o_bg_lab)
{
...
cv::Mat TransferI(cv::Mat fg_lab, cv::Mat fg_mask, float meanT[3], float stdT[3])
{
    ...
    for (int i = 0; i < BGR.rows; i++) {
        ...
        for (int j = 0; j < BGR.cols; j++) {
            for (int c = 0; c < BGR.channels(); c++) {
                ...
                if (bgrfScaled > 255.0)
                    bgrfScaled = 255.0;
                BGR_ptr[j * 3 + c] = (unsigned char)rndint(bgrfScaled);
            }
        }
    }
...
cv::Mat norm(const cv::Mat& originalI, float targetMean[3], float targetStd[3])
{
    ...
    PixelClass(originalI, o_fg, o_bg, o_fg_lab, o_bg_lab);
    ...
    cv::Mat fg_transferred = TransferI(o_fg_lab, o_fg, targetMean, targetStd);
    ...
    ColorNormI = lab2BGR(fg_transferred);
