...
int Segmentation::run()
{
	...
	if(inputRt != NULL){
		...
		if(bgr != NULL){
			...
			TaskSegmentation * segTask = new TaskSegmentation(bgr, mask, blue, green, red, T1, T2, G1, G2, minSize, maxSize, minSizePl, minSizeSeg, maxSizeSeg, fillHolesConnectivity, reconConnectivity, watershedConnectivity);
