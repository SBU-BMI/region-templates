...
struct HalGen {
    ...
    static std::vector<ArgumentBase *>
        _dft; // default empty constructor variable
    // must return an aborted flag (if true, all following stages must be
    // aborted)
...
class AutoStage : public RTPipelineComponentBase {
  ...
  public:
    ...
    AutoStage() { this->setComponentName("AutoStage"); };
...
class AutoStage {
  ...
  public:
    AutoStage(std::vector<RegionTemplate *> rts,
              std::vector<ArgumentBase *>   params,
              std::vector<int64_t> out_shape, std::list<HalGen *> schedules,
              Target_t target, int tileId)
        : rts(rts), params(params), out_shape(out_shape), last_stage(true),
          tileId(tileId) {
