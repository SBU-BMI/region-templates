...
// pthread_mutex_t ExecutionEngine::dependencyMapLock =
// PTHREAD_MUTEX_INITIALIZER;

ExecutionEngine::ExecutionEngine(int cpuThreads, int gpuThreads, int wid,
                                 int queueType, bool dataLocalityAware,
                                 bool prefetching) {
    schedType = queueType;
    ...
    threadPool->createThreadPool(cpuThreads, NULL, gpuThreads, NULL,
                                 dataLocalityAware, prefetching, wid);
...
bool ExecutionEngine::insertTask(Task *task) {
    ...
    // Resolve task dependencies and queue it for execution, or left the task
    // pending waiting
...
void ExecutionEngine::startupExecution() { threadPool->initExecution(); }

void ExecutionEngine::endExecution() {
    // this protection is used just in case the user calls this function
    // multiple times. It will avoid a segmentation fault
    if (threadPool != NULL) {
        tasksQueue->releaseThreads(threadPool->getGPUThreads() +
                                   threadPool->getCPUThreads());
        delete threadPool;
    }
    threadPool = NULL;
}
...
Task *ExecutionEngine::tryPreassignment(Task *task) {
    // forward message to track dependencies class
    return this->trackDependencies->tryPreAssignement(task, this->tasksQueue,
                                                      this->schedType);
}
...
void ExecutionEngine::startTransaction(CallBackTaskBase *transactionTask) {
    ...
    //		std::cout << "Error: calling startTranscation before ending
    // previous transaction (endTransaction)" <<std::endl;
    //	}
