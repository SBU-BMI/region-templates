...
class Task {
  private:
    ...
    // Auxiliary class variable used to assign an unique id to each class object
    // instance.
    ...
    // used to assert that a set of tasks were finalized before the computation
    // continues?
    int taskType;
    ...
    // This bool variable says whether the all dependencies that were to be
    // associated to this task have already being associated. It is only used by
    // the CallBackTaskClass. However, it has to be defined here because we
    // can't typecast the Task pointer available during the execution to an
    // object of type CallBackTask.
  ...
  public:
    ...
    // Called to increment the variable that keeps record of the number of
    // dependencies solved. The return is the value of dependencies solved after
    // incrementing it.
    ...
    void setSpeedup(int procType = ExecEngineConstants::GPU,
                    float speedup = 1.0);
