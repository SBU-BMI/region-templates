...
class ExecutionEngine {
  private:
    ...
    // Pool of threads that are consuming tasks from the associated queue of
    // tasks
    ThreadPool *threadPool;

    TrackDependencies *trackDependencies;

    // Used for thread-safe cout
    pthread_mutex_t w;
    ...
    //
    //	// Verifies if task dependencies have been solved and queue it for
    // execution,
    ...
    // Check if the argument task "task" will dispatch the execution of any
    // other dependent task, in that case try to perform a pre-assignement and
    // take advantage of the data location to
    // avoid unnecessary CPU-GPU data transfers
    Task *tryPreassignment(Task *task);
    //
    //	// This variable counts the number of tasks dispatched for execution
    // with the
    //	// execution engine that still pending waiting for a dependency to
    // finish 	int countTasksPending;
    //
    //	// Simply return the value of counting regading to number of tasks
    // pending
    ...
    // Grants access to these classes, they need to access private function
    // calls
    friend class Task;
    friend class ThreadPool;

  public:
    ExecutionEngine(int cpuThreads, int gpuThreads, int wid,
                    int queueType = ExecEngineConstants::FCFS_QUEUE,
                    bool dataLocalityAware = false, bool prefetching = false);
