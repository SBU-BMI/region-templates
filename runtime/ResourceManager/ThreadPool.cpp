...
void *callThread(void *arg) {
    ...
    if (procType == 2) {
        ...
        //	cv::cuda::setDevice(tid);
        //		if(tid==1){
        //			std::cout << "GPU thread: "<< tid << " using
        // GPU:"<< tid+1<<std::endl;
        // cv::cuda::setDevice(tid+1); 		}else{
        ...
        //
        //		if (sched_setaffinity(0, sizeof(cpu_set_t), &cpu_info)
        //== -1) { 			printf("Error: sched_getaffinity\n");
        //		}
        // warmup gpu
        cv::Mat A;
...
ThreadPool::~ThreadPool() {
    ...
    if (numCPUThreads + numGPUThreads > 1) {
        ...
        double tS =
            firstToFinishTime.tv_sec * 1000000 + (firstToFinishTime.tv_usec);
        double tE =
            lastToFinishTime.tv_sec * 1000000 + (lastToFinishTime.tv_usec);
...
bool ThreadPool::createThreadPool(int cpuThreads, int *cpuThreadsCoreMapping,
                                  int gpuThreads, int *gpuThreadsCoreMapping,
                                  bool dataLocalityAware, bool prefetching,
                                  int wid) {
...
#ifdef WITH_CUDA
void ThreadPool::preassignmentSelectiveDownload(Task *task, Task *preAssigned,
                                                cv::cuda::Stream &stream) {
    ...
    for (int i = 0; i < task->getNumberArguments(); i++) {
        ...
        for (j = 0; j < preAssigned->getNumberArguments(); j++) {
            ...
            if (task->getArgument(i)->getId() ==
                preAssigned->getArgument(j)->getId()) {
                break;
            }
        }
        // Did not find a matching, download
        if (j == preAssigned->getNumberArguments()) {
            //			std::cout << "Parameters i = "<< i <<" did not
            // match!"
            //<< std::endl;
            if (task->getArgument(i)->getType() != ExecEngineConstants::INPUT) {
                //				std::cout << "	Type is not
                // input = "<< task->getArgument(i)->getType() <<std::endl;
                task->getArgument(i)->download(stream);
                downloadingTasksIds.push_back(i);

            } else {
                //				std::cout << "	Type is input
                // only = "<< task->getArgument(i)->getType() <<std::endl;
                task->getArgument(i)->deleteGPUData();
            }
        }
    }
...
void ThreadPool::processTasks(int procType, int wid, int tid) {
    ...
    // Increment tasks semaphore to avoid the thread from get stuck when taking
    // the prefetched task
    ...
    afterGetTask:
        ...
        if (curTask == NULL) {
            ...
            if (this->execEngine->getCountTasksPending() != 0 ||
                this->execDone == false) {
                // Did not actually took a task, so increment the number of
                // tasks processed
                this->tasksQueue->incrementTasksToProcess();

#ifdef WITH_CUDA
                // if, for any reason, could not return a new tasks a
                // prefetchTask exist, compute the pre-fetched one
                if (prefetchTask != NULL) {
                    ...
                    //					std::cout << "Proc
                    // prefetched data, not enqueueing another"<<std::endl;

                    goto procPoint;
                } else {
                    // If there is nothing to compute, check whether there is an
                    // asynchronous download in course, and solve it. This may
                    // potentially create other tasks
                    if (downloadingTask != NULL) {
                        //						std::cout<<
                        //"No other tasks available... resolve
                        // asyncDownload"<<std::endl;
                        // wait until download is complete
                        dStream->waitForCompletion();

                        // release GPU memory space used by those downloaded
                        // parameters
                        deleteOutputParameters(downloadingTask);

                        // delete task being download, what will solve its
                        // dependencies
                        delete downloadingTask;

                        // set task to NULL, meaning that no other task is being
                        // downloaded
                ...
                // All tasks ready to execute are finished, but there still
                // existing tasks pending due unsolved dependencies
                continue;
            } else {
                // std::cout << "[ThreadPool][W" << wid << "][T" << tid <<
                // "][type"
                // 	<< procType << "] Task NULL #t_pending="
                // 	<< this->execEngine->getCountTasksPending() <<
                // std::endl;
                break;
            }
        } else {
            // It this tasks was canceled, resolve its dependencies and try to
            // get another task
            if (curTask->getStatus() != ExecEngineConstants::ACTIVE) {
                if (curTask->getTaskType() ==
                    ExecEngineConstants::TRANSACTION_TASK) {
            ...
            if (this->prefetching) {
                // Process task previously prefetched, and dispatch upload of
                // current task
                if (prefetchTask != NULL) {
                    // wait until data of the prefetched task is ready. We hope
                    ...
                    //					std::cout << "Proc
                    // prefetched data, enqueue another"<<std::endl;

                } else {
                    // If curTask is not NULL, and prefetch Taks is NULL there
                    // is a chance for performing the prefetching. This is the
                    // point where it startup
                    if (prefetchTask == NULL &&
                        procType == ExecEngineConstants::GPU) {
                        // set the curTask to the prefetching
                        prefetchTask = curTask;
                        enqueueUploadTaskParameters(prefetchTask, *pStream);
                        curTask = NULL;
                        //						std::cout
                        //<< "Enqueue prefetched data"<<std::endl;
                ...
                if (procType == ExecEngineConstants::GPU) {
                    // Not using prefetching, enqueue parameters upload and wait
                    // for completion
    ...
    procPoint:
        ...
        try {
            std::cout << "[ThreadPool][W" << wid << "][T" << tid << "][type"
                      << procType
                      << "] Executing, task.id: " << curTask->getId()
                      << std::endl;
            curTask->run(procType, tid);
            ...
            if (this->dataLocalityAware) {
                if (procType == ExecEngineConstants::GPU) {
                    //					std::cout << "AFTER run:
                    // isDataLocalityAware!"<<std::endl;
                    // 1) call pre-assignment.
                    preAssigned = curTask->tryPreassignment();
                }
            }
            // check whether there is an asynchronous download in course, and
            // solve it
            if (downloadingTask != NULL) {
                //				std::cout<< "After processing...
                // resolve asyncDownload"<<std::endl;
                // wait until download is complete
                dStream->waitForCompletion();
            ...
            // there is not task preAssigned, so download everything until the
            // next tasks is chosen for execution
            if (preAssigned == NULL) {
                if (procType == ExecEngineConstants::GPU) {
                    // if prefetching is not active, perform synchronous
                    // download
                    if (!prefetching) {
...
#endif
            // TODO: delete any available GPU data, and continue (what if the
            // CPU is cancelled)
            delete curTask;
            continue;
        }
...
// (tE-tS)/1000000);
        ...
        // October 04, 2013. Commenting out line bellow to work with GPUs
        // without compiling w/ cuda/gpu suppport
    ...
    std::string s = "[PROFILING][TASK_EXEC][W";
    s += std::to_string(mpi_rank);
    s += "][type";
    s += std::to_string(procType);
    s += "] ";
    s += std::to_string(sumOfExecTimes);
    s += "\n";
    this->execEngine->print(s);
    ...
    //	}
    // I don't care about controlling concurrent access here. Whether two
    // threads enter this if, their resulting gettimeofday will be very similar
    // if not the same.
    if (firstToFinish) {
        firstToFinish = false;
...
int ThreadPool::getGPUThreads() { return numGPUThreads; }
...
int ThreadPool::getCPUThreads() { return numCPUThreads; }
