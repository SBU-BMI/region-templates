...
void Worker::notifyComponentsCompleted()
{
	...
	if(this->getComputedComponentSize() > 0){
		...
		// Message format
		// |Mesg Type (char) | Number of Components to Pack (int) | the proper components data (id, resultSize, result) | number of components with extra data
		int message_size = sizeof(char) + sizeof(int);

		// add space for components data
		list<char*>::iterator itListCompCore = this->computedComponents.begin();
