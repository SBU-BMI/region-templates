...
#ifdef DEBUG
	std::cout << "move2Global: "<< rtName << " " << rtId << " " << drName << " "<<drId <<" "<< timestamp << " " << version << std::endl;
#endif
...
void Worker::addComputedComponent(int id, int dataSize, char *data)
{
...
void Worker::notifyComponentsCompleted()
{
	...
	if(this->getComputedComponentSize() > 0){
		...
		// Message format
		// |Mesg Type (char) | Number of Components to Pack (int) | the proper component ids (int * Number of Components to Pack) | number of components with extra data
		int message_size = sizeof(char) + sizeof(int) + number_components * sizeof(int) + sizeof(int);

		// save space for the components data size
		list<int>::iterator itListComp = this->computedComponentsDataSize.begin();
...
#ifdef DEBUG
		std::cout << "\tEXTRA data size = "<< message_size << " #components = "<< number_components << std::endl;
		std::cout << " computedComponents: " << number_components << " componentsDataSize: " << this->computedComponentsDataSize.size() << std::endl;
#endif
		if(this->computedComponentsDataSize.size() > 0){
			assert(this->computedComponentsDataSize.size() == number_components);
		}
		// Allocate Message
		char *msg = new char[message_size];
		...
		int_data_ptr[number_components+1] = this->computedComponentsDataSize.size();

		int packed_data_size = sizeof(char) + sizeof(int) + sizeof(int) * number_components + sizeof(int);
...
#ifdef DEBUG
			std::cout << "\t Worker: sizeCompData: "<< sizeCompData << std::endl;
#endif
			memcpy(msg+packed_data_size, &sizeCompData, sizeof(int));
			packed_data_size += sizeof(int);

			memcpy(msg+packed_data_size, compData, sizeof(char)*sizeCompData);
...
#ifdef WITH_RT
Cache* Worker::getCache() const {
	return cache;
}

void Worker::setCache(Cache* cache) {
	this->cache = cache;
}
#endif
