...
// sent message to Worker requesting for a data region to be send to global storage
		...
		if (comm_world.Iprobe(MPI_ANY_SOURCE, MessageTag::TAG_CONTROL, status)) {
			...
			switch(msg_type){
				...
				case MessageTag::WORKER_TASKS_COMPLETED:
				{
					...
					std::cout << "Manager: #CompCompleted = "<< number_components_completed << " time: "<< t1<<std::endl;

					// components with extra data
					int number_components_extra_data = tasks_data[number_components_completed+1];
