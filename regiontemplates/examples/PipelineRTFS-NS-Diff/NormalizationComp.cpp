...
int NormalizationComp::run()
{
	...
	if(inputRt != NULL){
		...
		if(raw != NULL){
			...
			std::string outputId = futils.replaceExt(id, ".tif", std::string("-").append(parameterId));
			outputId.append(".bgr.pbm");
//			std::string outputId = futils.replaceExt(id, ".tif", ".bgr.pbm");
			...
			bgr->setName("BGR");
			bgr->setId(outputId);
			bgr->setVersion(atoi(parameterId.c_str()));
			bgr->setOutputType(DataSourceType::FILE_SYSTEM);

			inputRt->insertDataRegion(bgr);

			int parameterIdInt = atoi(parameterId.c_str());
			// Create processing task
			TaskNormalization * normTask = new TaskNormalization(raw, bgr, parameterIdInt);
