			...
			if(it != rt->templateRegions.end()){
				...
				for(; listIt != (*it).second.end(); listIt++){
					...
					if(auxCache != NULL){
						...
						std::cout << "Stage data region into cache: "<< rtName << ":" << drName << " version:"<< dr->getVersion()<<" id: "<< dr->getId() <<std::endl;
					}else{
						std::cout << "AUXCACHE NULL" << std::endl;
					}

					//	}else{
					//		std::cout << "NOT STAGING DATA: "<< rtName << ":" << drName << " version:"<< dr->getVersion() << " EMPTY!!" << std::endl;
					//	}
				}
			}else{
