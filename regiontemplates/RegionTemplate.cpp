...
DataRegion* RegionTemplate::getDataRegion(std::string drName, std::string drId, int timestamp, int version) {
	...
	if(it != templateRegions.end()){
		...
		for(; listIt != (*it).second.end(); listIt++){
			...
			//		if(retValue->getIsAppInput() == true){
						//std::cout << "DataRegion is appInput: " << retValue->getName() << std::endl;
			//			DataRegionFactory::instantiateDataRegion(retValue, -1);
			//		}else{
						// data is in a cache layer.
